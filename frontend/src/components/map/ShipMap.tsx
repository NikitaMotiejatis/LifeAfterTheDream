  ...
  return (
    <div style={{ height: '100%', width: '100%', borderRadius: 8, overflow: 'hidden' }}>
      ...
      >
        ...
        />
        {vessels.map((v) => (
          <Marker key={v.mmsi} position={[v.lat, v.lon]} icon={shipIcon(v.cog, SHIP_COLORS[v.shiptype] ?? DEFAULT_COLOR)}>
            <Popup>
				<strong>{v.name}</strong><br />
				Type: {v.shiptype || '—'}<br />
				Speed: {v.sog} kn<br />
				Heading: {v.hdg === 0 ? 'N/A' : `${v.hdg}°`}<br />
      ...
      {isLoading && (
        <div style={{ position: 'absolute', top: 8, right: 8, background: '#fff', padding: '4px 8px', borderRadius: 4, fontSize: 11 }}>
          Loading...
        </div>
      )}
    </div>
