...
export default function ForecastSummaryCard({ data }: ForecastSummaryCardProps) {
  return (
    <div className="bg-white rounded-2xl shadow-md">
      <div className="p-6 border-b border-gray-200">
        <h2 className="text-xl font-semibold text-gray-900">Forecast Summary</h2>
          ...
          {data.map((item, idx) => (
            <div key={idx} className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusColors[item.statusColor]}`}>
                  {item.status}
                </span>
              </div>
